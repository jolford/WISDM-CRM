-- CRITICAL SECURITY FIXES - Part 4: Role escalation vulnerability fix (corrected)
...
BEGIN
  ...
  IF OLD.is_active != NEW.is_active AND (SELECT get_current_user_role()) != 'admin' THEN
...
-- Create secure role assignment function using TEXT type
...
BEGIN
  ...
  SELECT get_current_user_role()::text INTO current_admin_role;
