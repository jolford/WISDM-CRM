-- Fix the security definer view issue by recreating views without security_barrier
-- This ensures views use caller's permissions, not definer's permissions

-- Drop existing views
DROP VIEW IF EXISTS public.contacts_export_view;
DROP VIEW IF EXISTS public.deal_collections_summary;

-- Recreate contacts_export_view with proper user-scoped filtering
-- This view will automatically respect RLS from the underlying contacts table
...
-- Recreate deal_collections_summary view with proper user-scoped filtering
-- This view will automatically respect RLS from the underlying deals table
