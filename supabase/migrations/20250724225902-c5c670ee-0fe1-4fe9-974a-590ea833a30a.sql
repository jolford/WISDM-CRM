-- Fix Security Issues - Critical Fixes
...
-- 6. Add additional RLS policy to prevent role escalation
CREATE POLICY "Prevent self role escalation" ON public.profiles
  FOR UPDATE USING (
    -- Allow if user is admin OR if they're only updating personal info (not role/is_active)
    get_current_user_role() = 'admin'::app_role OR 
    (auth.uid() = id AND role = OLD.role AND is_active = OLD.is_active)
  );

-- 7. Create a safer view for contact imports (without SECURITY DEFINER)
...
WHERE c.user_id = auth.uid(); -- Only show user's own contacts

-- 8. Enable RLS on the new view
ALTER VIEW public.contacts_export_view OWNER TO postgres;

-- 9. Add RLS policy for the export view
CREATE POLICY "Users can view their own contact exports" ON public.contacts_export_view
